...
export default function HomeClient({ profile, about }: Props) {
  ...
  return (
    <div className="relative min-h-full w-full animate-in fade-in duration-600 bg-[var(--vsc-bg)]">
      {/* Hero Background Image with Hard Tint */}
      <div
      ...
      <div className="max-w-[95%] mx-auto relative z-10">
        ...
        <div className="mb-24 text-center lg:text-left">
          <div className="inline-flex items-center gap-3 mb-8 px-3 py-1 bg-[rgba(106,153,85,0.1)] border border-[rgba(106,153,85,0.2)] rounded-full">
            <span className="text-[var(--vsc-comment)] font-mono text-xs md:text-sm">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold text-[var(--vsc-text-bright)] leading-[1.1] mb-4 font-syne tracking-tighter">
          ...
          <div className="text-lg md:text-xl lg:text-2xl text-[var(--vsc-text)] font-mono mb-8 flex items-center justify-center lg:justify-start gap-3">
            ...
            <span className="text-[var(--vsc-string)]">&quot;{name}&quot;</span>
          </div>

          <p className="text-[var(--vsc-text-dim)] text-base md:text-lg max-w-[600px] leading-relaxed mb-10 font-sans mx-auto lg:mx-0">
          ...
          <div className="flex flex-wrap gap-4 justify-center lg:justify-start">
            <Link href="/projects" prefetch={true} className="btn-primary flex items-center gap-2 px-7 py-3 text-base font-semibold">
              <ArrowTopRightOnSquareIcon className="w-5 h-5" />
              Explore My Work
            </Link>
            <Link href="/contact" prefetch={true} className="btn-ghost px-7 py-3 text-base font-semibold bg-white/5">
              Contact Me
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-12 items-start pb-[40px]">
          {/* Editor Area */}
          <div className="glass-card rounded-xl h-full !bg-[rgba(30,30,30,0.5)] overflow-hidden border border-[var(--vsc-border)] ">
            <div className=" px-5 py-2.5 flex items-center justify-between border-b border-[var(--vsc-border)]">
              ...
              <span className="text-xs text-[var(--vsc-text-dim)] font-mono uppercase tracking-tighter opacity-50">
                {profile?.name?.toLowerCase().replace(/\s/g, '_') || "jahid_hasan"}.ts — portfolio
              </span>
            </div>
            <div className="py-6 bg-[rgba(30,30,30,0.5)]">
          ...
          <div className="flex flex-col gap-6 w-full">
            {stats.map(({ label, value, color, sub }) => (
              <div key={label} className="glass-card p-6 rounded-xl border border-[var(--vsc-border)] hover:border-[var(--vsc-accent)]/30 transition-colors">
                ...
                <div className="text-3xl md:text-4xl font-extrabold font-mono mb-1" style={{ color }}>
                  {value}
                </div>
                <div className="text-xs md:text-sm text-[var(--vsc-text-dim)] opacity-70">
                  {sub}
                </div>
              </div>
            ))}
